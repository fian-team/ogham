...
use ogham::tree::{event::Event, point::Point, UI};
use ogham::vm::Value;
...
impl App {
    ...
    fn new(ui_file: &str) -> Result<Self, RuntimeError> {
        ...
        host_state.insert("version".to_string(), Value::String("1.0.0".to_string()));

        // Example: Pass user data
        host_state.insert("user_name".to_string(), Value::String("Alice".to_string()));
        host_state.insert("user_id".to_string(), Value::Integer(12345));
        host_state.insert("is_premium".to_string(), Value::Boolean(true));

        // Example: Pass window dimensions (you'd update these on resize)
        host_state.insert("window_width".to_string(), Value::Integer(1024));
        host_state.insert("window_height".to_string(), Value::Integer(768));
